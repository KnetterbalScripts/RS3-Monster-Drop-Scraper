class MonsterScraperApp {
  ...
  setupEventListeners() {
    ...
    this.queueList.querySelectorAll('.queue-select-checkbox').forEach(cb => {
      cb.addEventListener('change', (e) => {
          const idx = Number(e.target.dataset.index);
          if (e.target.checked) this.selectedQueueIndexes.add(idx);
          else this.selectedQueueIndexes.delete(idx);
      });
    });

    // Voeg event listener toe voor groepsgewijze scraping
    const scrapeSelectedBtn = document.getElementById('scrapeSelectedButton');
    if (scrapeSelectedBtn) {
        scrapeSelectedBtn.addEventListener('click', () => this.startGroupScraping());
    }
  }
  ...
  async loadItemDatabase() {
    try {
      ...
      const statusText = `✅ Database loaded: ${dbInfo.itemCount.toLocaleString()} items (${
        dbInfo.type
      })`;
      this.databaseStatus.textContent = statusText;
      this.databaseInfo.textContent = `Database: ${dbInfo.itemCount.toLocaleString()} items (${
        dbInfo.type
      })`;
  ...
  updateQueueDisplay() {
    if (!this.selectedQueueIndexes) this.selectedQueueIndexes = new Set();
    ...
    this.monsterQueue.forEach((monster, index) => {
        ...
        queueItem.innerHTML = `
            <input type="checkbox" class="queue-select-checkbox" data-index="${index}" ${this.selectedQueueIndexes.has(index) ? 'checked' : ''} style="margin-right:10px;">
    ...
    const scrapeSelectedBtn = document.getElementById('scrapeSelectedButton');
    if (scrapeSelectedBtn) {
        scrapeSelectedBtn.onclick = () => this.startGroupScraping();
    }
  }
  ...
  async startScraping() {
    ...
    if (this.monsterQueue.length === 0) {
      this.showStatus(
        "Please add at least one monster to the queue",
        "warning"
      );
      return;
    }
    ...
    try {
      ...
      for (let i = 0; i < this.monsterQueue.length; i++) {
        ...
        this.showStatus(
          `🔍 Scraping ${monster.name} (${i + 1}/${totalMonsters})...`,
          "info"
        );
        this.showProgress(progress);

        try {
          const drops = await window.electronAPI.scrapeMonster(
            monster.url,
            monster.name
          );
      ...
      this.showStatus(
        `✅ Completed scraping ${totalMonsters} monsters`,
        "success"
      );
    } catch (error) {
  ...
  linkItemsToIds(drops) {
    ...
    for (const drop of drops) {
      const baseItemName = drop.itemName
        .toLowerCase()
        .trim()
        .replace(/\s*\(noted\)\s*$/i, "")
        .trim();
      const isNoted =
        drop.itemName.toLowerCase().includes("(noted)") || drop.isNoted;
      ...
      if (isNoted) {
        ...
        const baseItem = this.itemDatabase.items?.find(
          (item) =>
            item.name &&
            item.name.toLowerCase() === baseItemName &&
            item.noteData
        );
          ...
          if (!itemId) {
            ...
            if (unnotedId) {
              ...
              if (
                ...
                this.itemDatabase.items.find(
                  (item) => item.id === potentialNotedId
                )
              ) {
      ...
      if (itemId && !seenUniqueItems.has(uniqueKey)) {
        ...
        const cleanItemName = drop.itemName
          .replace(/\s*\(noted\)\s*$/i, "")
          .trim();
  ...
  findTradeableItemId(itemName, items) {
    ...
    const matchingItems = items.filter((item) => {
      ...
      const baseItemName = itemNameLower
        .replace(/\s*\(noted\)\s*$/i, "")
        .trim();
    ...
    results.forEach((result, index) => {
      ...
      resultItem.innerHTML = `
                <div class="result-header" style="flex-direction: column;">
                    <div style="display: flex; justify-content: space-between; align-items: center; width: 100%;">
                        <div>
                            <div class="result-monster">${statusIcon} ${this.escapeHtml(
        result.monster
      )}</div>
                ...
                <div class="result-drops" id="result-drops-${index}" style="margin-top:12px; display: ${
        result.totalFoundDrops > 0 ? "block" : "none"
      };">
                    <!-- Controls -->
      ...
      if (result.drops && result.drops.length > 0) {
        ...
        result.drops.forEach((drop, di) => {
          ...
          tdName.innerHTML = `<strong>${this.escapeHtml(
            drop.itemName
          )}</strong>${
            drop.isNoted
              ? ' <span style="color:#f0ad4e; font-weight:600;">(noted)</span>'
              : ""
          }`;
        ...
        const deselectAllBtn = resultItem.querySelector(
          `#deselect-all-${index}`
        );
  ...
  async downloadResult(index) {
    ...
    const fileName = `${result.monster
      .toLowerCase()
      .replace(/[^a-z0-9]/g, "_")}_drops.json`;

    try {
      const filePath = await window.electronAPI.saveDrops(
        result.monster,
        result.drops
      );
      this.showStatus(
        `✅ Saved ${result.monster} drops to: ${filePath}`,
        "success"
      );
    } catch (error) {
  ...
  async downloadResultLua(index) {
    ...
    const result = this.lastResults[index];
    // Check if this is a group result (monster name starts with 'Group (')
    if (result.monster && result.monster.startsWith('Group (') && Array.isArray(result.drops)) {
      // Use saveGroupLua for group output
      // Try to get monsterNames from the queue if possible, else fallback to empty array
      let monsterNames = [];
      if (this.selectedQueueIndexes && this.monsterQueue) {
        monsterNames = Array.from(this.selectedQueueIndexes).map(idx => this.monsterQueue[idx]?.name).filter(Boolean);
      }
      if (monsterNames.length === 0 && result.monsterNames) {
        monsterNames = result.monsterNames;
      }
      try {
        const filePath = await window.electronAPI.saveGroupLua(monsterNames, result.drops);
        this.showStatus(
          `✅ Saved group drops to Lua: ${filePath}`,
          "success"
        );
      } catch (error) {
        console.error("Failed to save group Lua file:", error);
        this.showStatus("Failed to save group Lua file: " + error.message, "error");
      }
    } else {
      // Single monster: use saveDropsLua
      try {
        const filePath = await window.electronAPI.saveDropsLua(
          result.monster,
          result.drops
        );
        this.showStatus(
          `✅ Saved ${result.monster} drops to Lua: ${filePath}`,
          "success"
        );
      } catch (error) {
  ...
  async downloadResultLuaSelected(index) {
    ...
    const selectedIds =
      result.selected && result.selected.size > 0 ? result.selected : null;
    ...
    if (result.monster && result.monster.startsWith('Group (') && Array.isArray(result.drops)) {
      ...
      try {
        const filePath = await window.electronAPI.saveGroupLua(monsterNames, selectedDrops);
        this.showStatus(
          `✅ Saved group selected drops to Lua: ${filePath}`,
          "success"
        );
      } catch (error) {
        console.error("Failed to save group selected Lua file:", error);
        this.showStatus("Failed to save group selected Lua file: " + error.message, "error");
      }
    } else {
      // Single monster: use saveDropsLua
      try {
        ...
        this.showStatus(
          `✅ Saved ${result.monster} selected drops to Lua: ${filePath}`,
          "success"
        );
      } catch (error) {
  ...
  async startGroupScraping() {
        if (!this.selectedQueueIndexes || this.selectedQueueIndexes.size === 0) {
            this.showStatus('Select at least one monster in the queue', 'warning');
            return;
        }
        if (this.isScrapingActive) {
            this.showStatus('Scraping is already in progress', 'warning');
        ...
        this.addToQueueBtn.disabled = true;
        const scrapeSelectedBtn = document.getElementById('scrapeSelectedButton');
        if (scrapeSelectedBtn) scrapeSelectedBtn.disabled = true;
        ...
        try {
            ...
            const selectedMonsters = Array.from(this.selectedQueueIndexes).map(idx => this.monsterQueue[idx]);
            ...
            // Combine all drops and filter duplicates
            const uniqueDropsMap = new Map();
            for (const drop of allDrops) {
                const key = `${drop.itemId}_${drop.isNoted ? 'noted' : 'unnoted'}`;
                if (!uniqueDropsMap.has(key)) {
                    uniqueDropsMap.set(key, drop);
                }
            }
            const uniqueDrops = Array.from(uniqueDropsMap.values());

            // Show result as one group
            this.displayResults([
                {
                    monster: `Group (${totalMonsters} monsters)`,
                    url: '',
                    scrapedAt: new Date().toISOString(),
                    totalFoundDrops: uniqueDrops.length,
                    drops: uniqueDrops
                }
            ]);

            // Auto-save as group (JSON)
            if (uniqueDrops.length > 0) {
                try {
                    await window.electronAPI.saveDrops('group', uniqueDrops);
                } catch (error) {
                    console.error('Failed to save group drops:', error);
                }
            }
            // Auto-save as group (Lua)
            if (uniqueDrops.length > 0 && monsterNames.length > 0) {
                try {
                    await window.electronAPI.saveGroupLua(monsterNames, uniqueDrops);
                } catch (error) {
                    console.error('Failed to save group Lua drops:', error);
                }
            }

            this.showStatus(`✅ Group scraping completed (${totalMonsters} monsters)`, 'success');

        } catch (error) {
            console.error('Group scraping failed:', error);
            this.showStatus('Group scraping failed: ' + error.message, 'error');
        } finally {
            this.isScrapingActive = false;
            this.scrapeBtn.disabled = false;
            this.addToQueueBtn.disabled = false;
            if (scrapeSelectedBtn) scrapeSelectedBtn.disabled = false;
            this.progressContainer.style.display = 'none';
        }
    }
